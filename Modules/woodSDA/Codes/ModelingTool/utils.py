...
def defineNodes3DModel(ModelDirectory, BuildingModel):
  ...
  with open('DefineNodes3DModel.tcl', 'w') as tclfile:
    ...
    # loop through the number of stories (z axis) adding panels per story
    for i in range (1, BuildingModel.numberOfStories+1):
      tclfile.write("# Story %i \n"%i)
      # loop through the number of x directionpanels
      # numberOfXDIrectionWoodPanel = list of two number eg: 16 and 29 for story i and i+1
...
# similarly define the nodes for z direction wood panel. z here refers to opensees y coord? 
    tclfile.write('# Define z-direction wood panel nodes \n')
    
    #loop through the number of floors to add  nodes for z-dir panels per floor 
    for i in range (1, BuildingModel.numberOfStories+1):
      tclfile.write("# Story %i \n"%i)
      ## numerOfZDirectionWoodPanels = list of two numbers for each story 
...
# leaning columcolumnnodestag = opensees node tag for each floor for no of leaning column (3x9) for 3 story bldg
    NumLeaningCol = BuildingModel.leaningColumnNodesOpenSeesTags.shape[1]

    for j in range(NumLeaningCol):
        # produce 3 nodes for 2 story, thus +2
...
def setupPushoverAnalysis(ModelDirectory, BuildingModel):
  ...
  with open('RunXPushoverAnalysis.tcl', 'w') as tclfile:
    ...
    tclfile.write('set\tRunTime\t[expr ($endT - $startT)]\n')
...
def generatePushoverAnalysisModel(ID, BuildingModel, BaseDirectory, DB_Directory, GenerateModelSwitch = True, RunPushoverSwitch = False):
    ModelDirectory = BaseDirectory + '/BuildingModels/%s'%ID
#     InfoDir = os.path.join(BaseDirectory + 'BuildingInfo')
    ...
    if RunPushoverSwitch:
        ...
        os.system('/Applications/OpenSees/bin/OpenSees RunXPushoverAnalysis.tcl')
        os.system('/Applications/OpenSees/bin/OpenSees RunZPushoverAnalysis.tcl')
