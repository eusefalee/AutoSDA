...
def calculate_spectral_acceleration_parameters(Ss, S1, Fa, Fv, T):
    """
    ...
    :param T: a scalar to denote the building period
    :return: SMS, SM1, SDS, SD1: four scalars which are required for lateral force calculation
    """
    # Compute MCE spectral acceleration parameters
    SMS = Fa * Ss
    SM1 = Fv * S1
    # Compute design spectral acceleration parameters
    SDS = 2/3 * SMS
    SD1 = 2/3 * SM1
    # Compute design spectra
    T0 = 0.2 * SD1/SDS
    TS = SD1/SDS
    TL = 8.0
    if T <= T0:
        SaDBE = SDS * (0.4 + 0.6 * T/T0)
    elif T <= TS:
        SaDBE = SDS
    elif T <= TL:
        SaDBE = SD1 / T
    ...
    # Compute SaMCE
    SaMCE = 1.5 * SaDBE
    return SMS, SM1, SDS, SD1, SaDBE, SaMCE
...
def compute_SaMCE_scaling_factor(SaMCE, T, acceleration_spectrum):
    """
    This function is used to compute the ratio of SaMCE to the median intensity of the given ground motion at the target
    period. This ratio is further is used to anchor the median ground motion Sa to MCE level
    :param SaMCE: a scalar to denote the building's SaMCE value
    :param T: a scalar to denote the building's period
    :param spectral_acceleration: a dataframe: the 1st column is time series and rest columns are spectral acceleration
    :return: a scalar that denote the ratio of SaMCE to the median of spectral acceleration
    """
    row, col = acceleration_spectrum.shape
    acceleration_spectrum = acceleration_spectrum.values
    all_Sa = list()
    # Use linear interpolation to compute the Sa values for each ground motion at target period
    for index in range(0, col-1):
        linear_function = interpolate.interp1d(acceleration_spectrum[:, 0], acceleration_spectrum[:, 1+index])
        SaGM = linear_function(T)
        all_Sa.append(SaGM)
    # Compute the ratio of SaMCE to the median Sa value of all ground motions
    SaGM = np.median(all_Sa)
    ratio = SaMCE / SaGM
    return ratio


def convert_number_list_to_string(all_scales):
    """
    This method is used to convert a Python number list to a string with space as delimiter.
    :param all_scales: a Python list with numbers
    :return: a string of all numbers with space as delimiter
    """
    result = "{"
    for each in all_scales:
        result += str(each)
        if (each != all_scales[-1]):
            result += " "
        else:
            result += "}"
    return result
...
def determine_spectral_shape_factor(ductility, T):
    """
    This method is used to determine the spectral shape factor (SSF) based on FEMA P695 Table 7.1b.
    :param ductility: a scalar to denote the ductility ratio
    :param T: a scalar to denote the period
    :return: a scalar to denote the SSF
    """
    # FEMA P695 Table 7-1b:
    # Define table header
    x = [1, 1.1, 1.5, 2, 3, 4, 6, 8, 20]
    # Define the first column: periods
    y = [0.0, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 8.0]
    # Define the table content
    z = np.array([[1, 1.05, 1.10, 1.13, 1.18, 1.22, 1.28, 1.33, 1.33],
                 [1, 1.05, 1.10, 1.13, 1.18, 1.22, 1.28, 1.33, 1.33],
                 [1, 1.05, 1.11, 1.14, 1.20, 1.24, 1.30, 1.36, 1.36],
                 [1, 1.06, 1.11, 1.15, 1.21, 1.25, 1.32, 1.41, 1.41],
                 [1, 1.06, 1.12, 1.16, 1.22, 1.27, 1.35, 1.41, 1.41],
                 [1, 1.06, 1.13, 1.17, 1.24, 1.29, 1.37, 1.44, 1.44],
                 [1, 1.07, 1.13, 1.18, 1.25, 1.31, 1.39, 1.46, 1.46],
                 [1, 1.07, 1.14, 1.19, 1.27, 1.32, 1.41, 1.49, 1.49],
                 [1, 1.07, 1.15, 1.20, 1.28, 1.34, 1.44, 1.52, 1.52],
                 [1, 1.08, 1.16, 1.21, 1.29, 1.36, 1.46, 1.55, 1.55],
                 [1, 1.08, 1.16, 1.22, 1.31, 1.38, 1.49, 1.58, 1.58],
                 [1, 1.08, 1.17, 1.23, 1.32, 1.40, 1.51, 1.61, 1.61],
                 [1, 1.08, 1.17, 1.23, 1.32, 1.40, 1.51, 1.61, 1.61]])
    # Perform 2D linear interpolation
    linear_function = interpolate.interp2d(x, y, z, kind='linear')
    result = linear_function(ductility, T).item()
    return result