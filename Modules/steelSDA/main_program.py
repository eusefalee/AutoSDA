...
# IDs = [1]
    for id in IDs:
        ...
        # These lines are added to revise the R factor based on Google Drive input.
        # os.chdir(base_directory / 'BuildingData' / building_id)
        # ELF_parameters = pd.read_csv('ELFParameters.csv', header=0)
        # ELF_parameters['R'] = R_factor
        # ELF_parameters.to_csv('ELFParameters.csv', sep=',', index=False)
        # Google drive data exchange ends here

        print("Design for Building ID = ", id)
        building,_ = seismic_design(building_id, base_directory)
        ...
        _,steel_weight = building.compute_steel_weight()
