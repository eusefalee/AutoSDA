...
from help_functions import determine_spectral_shape_factor
...
class PostprocessNonlinearAnalysis(object):
    ...
    def __init__(self, building_directory, data_directory, number_story, GM_IDs, IDA_scales, SaMCE, SaMCE_factor, period):
        ...
        self.intensity_levels = SaMCE * SaMCE_factor * np.array(IDA_scales) / 100
        ...
        self.save_EDPs(data_directory)
        self.process_peak_story_drift(IDA_scales, GM_IDs)
        self.save_IDA_results(data_directory, IDA_scales)
        self.count_collapse_case()
        self.lognormfit(len(GM_IDs), 'MLE')
        self.compute_performance_metrics(SaMCE, period)
    ...
    def save_EDPs(self, data_directory):
        """
        ...
        writer = pd.ExcelWriter('PeakStoryDrift.xlsx')
        ...
        writer = pd.ExcelWriter('PeakFloorAcceleration.xlsx')
    ...
    def save_IDA_results(self, data_directory, IDA_scales):
        """
        ...
        dataframe.to_csv('IDAMaxDrift.csv', sep=',', index=False)
    ...
    def lognormfit (self, number_of_GMs, Method='MLE'):
        """
    ...
    def compute_performance_metrics(self, SaMCE, period):
        """
        ...
        self.performance_metrics['ACMR'] = ACMR
