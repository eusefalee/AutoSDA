...
def save_design_drifts(building, constructability):
    ...
    if DESIGN_METHOD == 'ELF':
        final_design_drift = pd.DataFrame(data=building.elastic_response['story drift']*building.elf_parameters['Cd'],
                                          columns=['story drift'])
        final_design_drift.to_csv(DESIGN_METHOD + prefix + 'DriftProfile.csv', sep=',', index=False)
        final_displacement = pd.DataFrame(data=building.elastic_response['floor displacement']*building.elf_parameters['Cd'],
                                          columns=['Floor displacement'])
        final_displacement.to_csv(DESIGN_METHOD + prefix + 'FloorDisplacement.csv', sep=',', index=False)
    else:
        final_design_drift = pd.DataFrame(data=building.elastic_response['story drift'],
                                          columns=['story drift'])
        final_design_drift.to_csv(DESIGN_METHOD + prefix + 'DriftProfile.csv', sep=',', index=False)
        final_displacement = pd.DataFrame(data=building.elastic_response['floor displacement'],
                                          columns=['Floor displacement'])
        final_displacement.to_csv(DESIGN_METHOD + prefix + 'FloorDisplacement.csv', sep=',', index=False)
...
def store_RBS_dimension(building, beam_set, constructability):
    """
    This function is used to store the RBS dimensions for all buildings
    :param building: a class defined in "building_information.py" file.
    :param beam_set: a set of beam instances for the entire building.
    :param constructability: a boolean variable to denote whether to consider the constructability.
    :return: No variables. Just save RBS dimensions into .csv files.
    """
    prefix = 'Construction' if constructability else 'Optimal'
    RBS_dimensions = np.zeros([building.geometry['number of story'], 3])
    # Note that the identical beam sizes are used throughout one entire floor level -> RBS is the same for one floor.
    for story in range(0, building.geometry['number of story']):
        RBS_dimensions[story, :] = [beam_set[story][0].RBS_dimension['a'], beam_set[story][0].RBS_dimension['b'],
                                    beam_set[story][0].RBS_dimension['c']]
    df = pd.DataFrame(data=RBS_dimensions, columns=['a', 'b', 'c'])
    df.to_csv(prefix + 'RBSDimensions.csv', sep=',', index=False)



def store_miscellaneous_design_results(stability_coefficients, modal_periods, ELF_story_shear_forces_strength,
                                       ELF_story_shear_forces_drift, spectral_parameters, design_method,
                                       RSA_story_shear_forces_strength=None,
                                       RSA_story_shear_forces_drift=None, modal_shapes=None):
    """
    This method is used to save the stability coefficients.
    :param stability_coefficients: a N*1 numpy array where N is the number of stories for the building.
    :param modal_periods: a N*1 numpy array where N is the number of modes (equal to number of stories)
    :param ELF_story_shear_forces_strength: a N*1 numpy array where N is the number of stories
    :param ELF_story_shear_forces_drift: a N*1 numpy array where N is the number of stories
    :param spectral_parameters: a dataframe which includes all ELF-related spectral parameters
    :param RSA_story_shear_forces_strength: a N*1 numpy array where N is the number of stories
    :param RSA_story_shear_forces_drift: a N*1 numpy array where N is the number of stories
    :param modal_shapes: a N*M numpy array where N is the number of stories and M is the number of modes
    :return: No variables. Just save the rest design results into .csv files.
    """
    # Store the stability coefficients
    df_1 = pd.DataFrame(data=stability_coefficients, columns=['Stability coefficient'])
    df_1.to_csv('StabilityCoefficient.csv', sep=',', index=False)
    # Store the modal periods (not the ASCE formula-based period)
    df_2 = pd.DataFrame(data=modal_periods, columns=['Modal periods'])
    df_2.to_csv('ModalPeriods.csv', sep=',', index=False)
    # Store the ELF lateral forces computed based on ASCE 7 Section 12.8
    # There are two sets of forces: one for the strength and the other for drift/deformation
    df_3 = pd.DataFrame(data=ELF_story_shear_forces_strength, columns=['Story shear forces'])
    df_3.to_csv('ELFStoryShearForcesStrength.csv', sep=',', index=False)
    df_4 = pd.DataFrame(data=ELF_story_shear_forces_drift, columns=['Story shear forces'])
    df_4.to_csv('ELFStoryShearForcesDrift.csv', sep=',', index=False)
    # Store the spectral parameters
    spectral_parameters.to_csv('ELFSpectralParameters.csv', sep=',', index=True)
    # Store the RSA design story shear forces
    if design_method != 'ELF':
        df_6 = pd.DataFrame(data=RSA_story_shear_forces_strength, columns=['Story shear forces'])
        df_6.to_csv('RSAStoryShearForcesStrength.csv', sep=',', index=False)
        df_7 = pd.DataFrame(data=RSA_story_shear_forces_drift, columns=['Story Shear forces'])
        df_7.to_csv('RSAStoryShearForcesDrift.csv', sep=',', index=False)
        number_of_story = len(RSA_story_shear_forces_drift)
        header = ['Mode %i' % i for i in range(1, number_of_story+1)]
        df_8 = pd.DataFrame(data=np.array(modal_shapes).T, columns=header)
        df_8.to_csv('ModalShapes.csv', sep=',', index=False)

def save_scaling_factor(scaling_factor_force, scaling_factor_drift):
    """
    This function is used to save the scaling factor for the force and drift used in MRSA
    :param scaling_factor_force: a scalar denote the scaling factor for the force
    :param scaling_factor_drift: a scalar denote the scaling factor for the drift
    :return: no return variables. Just save to a .csv file
    """
    df = pd.DataFrame(data=[[scaling_factor_force, scaling_factor_drift]], columns=['Strength', 'Drift'])
    df.to_csv('RSAScalingFactor.csv', sep=',', index=False)
...
def save_all_design_results(building, column_set, beam_set, connection_set, constructability):
    ...
    # Store the RBS dimensions
    store_RBS_dimension(building, beam_set, constructability)
