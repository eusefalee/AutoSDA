...
from help_functions import calculate_spectral_acceleration_parameters
...
class Building(object):
    ...
    def read_geometry(self):
        ...
        geometry_data = pd.read_csv('Geometry.csv', header=0)
    ...
    def read_gravity_loads(self):
        ...
        """
        os.chdir(self.directory['building data'])
        loads_data = pd.read_csv('Loads.csv', header=0)
    ...
    def read_elf_parameters(self):
        ...
        """
        os.chdir(self.directory['building data'])
        elf_parameters_data = pd.read_csv('ELFParameters.csv', header=0)
        ...
        approximate_period = elf_parameters_data.loc[0, 'Ct'] \
                             * (self.geometry['floor height'][-1].item()**elf_parameters_data.loc[0, 'x'])
        ...
        # Overwrite the Fa and Fv values
        # If SDC = E
        # Fa = 1.2
        # Fv = 1.4

        # Determine SMS, SM1, SDS, SD1 using the defined function in "help_functions.py"
        SMS, SM1, SDS, SD1, SaDBE, SaMCE = calculate_spectral_acceleration_parameters(elf_parameters_data.loc[0, 'Ss'],
                                                                                      elf_parameters_data.loc[0, 'S1'],
                                                                                      Fa, Fv, approximate_period)
    ...
    def read_modal_period(self):
        ...
        self.elf_parameters['modal period'] = np.array(period).item((0, 0))
    ...
    def read_floor_displacement(self):
        """
        This method is used to read the floor displacements from OpenSees elastic analysis results.
        :return: an [story+1]*1 array which includes the floor displacements.
        """
        path_story_drift = self.directory['building elastic model'] / 'GravityEarthquake' / 'NodeDisplacements'
        os.chdir(path_story_drift)
        # Save all story drifts in an array
        floor_displacement = np.zeros([self.geometry['number of story']+1, 1])
        for floor in range(0, self.geometry['number of story']+1):
            file_name = 'NodeDisplacementLevel' + str(floor+1) + '.out'
            read_data = np.loadtxt(file_name)
            floor_displacement[floor] = read_data[-1, 1]
        # Present the results
        self.elastic_response['floor displacement'] = floor_displacement
        return floor_displacement
    ...
    def compute_steel_weight(self):
        """
        This method is used to compute the steel weight
        :return: a scalar to describe the steel weight for the entire frame in the unit of pounds
        """
        weight = 0
        floor_weight = list()
        # Compute the summation of exterior column, interior column, and beam
        for story in range(0, self.geometry['number of story']):
            exterior_column_weight = extract_weight(self.member_size['exterior column'][story])
            interior_beam_weight = extract_weight(self.member_size['interior column'][story])
            beam_weight = extract_weight(self.member_size['beam'][story])
            # If it is the first story: use first story height
            if story == 0:
                # There are two exterior columns in each story
                each_story_exterior_column = exterior_column_weight * self.geometry['first story height'] * 2
                # There are (number of bay - 1) interior columns in each story
                each_story_interior_column = interior_beam_weight * self.geometry['first story height'] * \
                                             (self.geometry['number of X bay']-1)
            # If it is upper story: use typical story height
            else:
                each_story_exterior_column = exterior_column_weight * self.geometry['typical story height'] * 2
                each_story_interior_column = interior_beam_weight * self.geometry['typical story height'] * \
                                             (self.geometry['number of X bay']-1)
            each_floor_beam = beam_weight * self.geometry['X bay width'] * self.geometry['number of X bay']
            # Get the summation of beams and columns in one story/floor level
            floor_weight.append(each_story_exterior_column + each_story_interior_column + each_floor_beam)
            weight += each_story_exterior_column + each_story_interior_column + each_floor_beam
        return weight.item(), floor_weight