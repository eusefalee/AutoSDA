...
# ********************************************* May 5, 2021 ************************************************************
from global_variables import BEAM_STIFFNESS_REDUCTION
# ********************************************* May 5, 2021 Ends *******************************************************
...
class ElasticAnalysis(object):
    ...
    def __init__(self, building, design_method, analysis_load_type, which_mode=None, lateral_forces=None):
        """
        ...
        :param design_method: a string: 'RSA' or 'ELF'.
        :param analysis_load_type: a string.
                                    if design_method == 'ELF', analysis_load_type could be 'EigenValue', 'DeadLoad',
                                    'LiveLoad', 'EarthquakeLoad', 'GravityEarthquake', 'All'
                                    if design_method == 'RSA', analysis_load_type could be any value mentioned above
                                    except 'All'.
        """
        ...
        # If the analysis is only eigen value analysis, then it is not necessary to generate the following files.
        if analysis_load_type != 'EigenValue':
            ...
            self.write_reaction_recorder(building)
            self.write_gravity_dead_load(building)
            self.write_gravity_live_load(building)
            self.write_earthquake_load(building, design_method, analysis_load_type, lateral_forces)
            self.write_gravity_earthquake_load(building)
        self.copy_baseline_files(building, design_method, analysis_load_type, which_mode)
    ...
    def write_nodes(self, building):
        ...
        with open('DefineNodes2DModel.tcl', 'w') as tclfile:
            ...
            for i in range(1, building.geometry['number of story']+2):
                tclfile.write("node\t%i%i%i" % (9, building.geometry['number of X bay']+2, i))  # Node label
            ...
            for i in range(2, building.geometry['number of story']+2):
                ...
                tclfile.write("node\t%i%i%i%i" % (9, building.geometry['number of X bay']+2, i, 2))  # Node label
                tclfile.write("\t[expr %i*$BayWidth]" % (building.geometry['number of X bay'] + 1))  # X coordinate
                ...
                if i < building.geometry['number of story']+1:
                    ...
                    tclfile.write("node\t%i%i%i%i" % (9, building.geometry['number of X bay']+2, i, 4))  # Node label
                    tclfile.write("\t[expr %i*$BayWidth]" % (building.geometry['number of X bay']+1))  # X coordinate
                    tclfile.write("\t[expr 1*$FirstStory+%i*$TypicalStory];" % (i-2))  # Y coordinate
        ...
        with open('DefineFixities2DModel.tcl', 'w') as tclfile:
            ...
            for j in range(1, building.geometry['number of X bay']+2):
                if COLUMN_BASE == 'FIX':
                    ...
                    tclfile.write("fix\t%i%i%i\t1\t1\t0; \n" % (j, 1, 1))
            # Leaning column base
            tclfile.write("fix\t%i%i%i\t1\t1\t0; \n\n" % (9, building.geometry['number of X bay']+2, 1))
        ...
        with open('DefineFloorConstraint2DModel.tcl', 'w') as tclfile:
            ...
            for i in range(2, building.geometry['number of story']+2):
                ...
                tclfile.write("equalDOF\t%i%i%i\t%i%i%i\t$ConstrainDOF;"
                              % (1, i, 1, 9, building.geometry['number of X bay']+2, i))
        ...
        with open('DefineBeams2DModel.tcl', 'w') as tclfile:
            ...
            for i in range(2, building.geometry['number of story']+2):
                ...
                for j in range(1, building.geometry['number of X bay']+1):
                    ...
                    tclfile.write("\t[expr %f*[lindex $BeamLevel%i 6]]" % (BEAM_STIFFNESS_REDUCTION, i))
                    # ********************************************* May 5, 2021 Ends ***********************************
                ...
                tclfile.write("\t%i%i%i" % (9, building.geometry['number of X bay']+2, i))  # Ending node in leaning column
        ...
        with open('DefineColumns2DModel.tcl', 'w') as tclfile:
            ...
            for i in range(1, building.geometry['number of story']+1):
                ...
                if i == 1:
                    ...
                    tclfile.write("\t%i%i%i" % (9, building.geometry['number of X bay']+2, i))
                    tclfile.write("\t%i%i%i%i" % (9, building.geometry['number of X bay']+2, i+1, 2))
                else:
                    tclfile.write("\t%i%i%i%i%i%i%i" % (3, building.geometry['number of X bay']+2, i, 4,
                                                        building.geometry['number of X bay']+2, i+1, 2))
                    tclfile.write("\t%i%i%i%i" % (9, building.geometry['number of X bay']+2, i, 4))
                    tclfile.write("\t%i%i%i%i" % (9, building.geometry['number of X bay']+2, i+1, 2))
    ...
    def write_leaning_column_spring(self, building):
        ...
        with open('DefineLeaningColumnSpring.tcl', 'w') as tclfile:
            ...
            for i in range(2, building.geometry['number of story']+2):
                ...
                tclfile.write("\t%i%i%i" % (9, building.geometry['number of X bay']+2, i))  # Node at floor level
                tclfile.write("\t%i%i%i%i;" % (9, building.geometry['number of X bay']+2, i, 2))  # Node below floor level
    ...
    def write_all_recorder(self):
        ...
        with open('DefineAllRecorders2DModel.tcl', 'w') as tclfile:
            ...
            tclfile.write("cd\t$baseDir\n")
            tclfile.write("source\tDefineNodeReactionRecorders2DModel.tcl\n\n")
    ...
    def write_reaction_recorder(self, building):
        # Create a .tcl file to write base reaction recorder for output
        with open('DefineNodeReactionRecorders2DModel.tcl', 'w') as tclfile:
            tclfile.write("# Define base reaction recorders\n\n\n")
            tclfile.write("cd\t$baseDir/$dataDir/BaseReactions\n\n")
            # Write the node reaction recorder for node at each ground level
            tclfile.write("recorder\tNode\t-file")
            tclfile.write("\tNodeReactionLevel%i.out" % 1)
            tclfile.write("\t-time\t-node")
            for j in range(1, building.geometry['number of X bay'] + 2):
                tclfile.write("\t%i%i%i" % (j, 1, 1))
            tclfile.write("\t%i%i%i" % (9, building.geometry['number of X bay'] + 2, 1))
            tclfile.write("\t-dof\t1\t2\t3\treaction; \n")
    ...
    def write_gravity_dead_load(self, building):
        ...
        with open('DefineGravityDeadLoads2DModel.tcl', 'w') as tclfile:
            ...
            for i in range(2, building.geometry['number of story']+2):
                tclfile.write("load\t%i%i%i\t0\t[expr -1*$LeaningColumnDeadLoadFloor%i]\t0; \n"
                              % (9, building.geometry['number of X bay']+2, i, i))
            tclfile.write("\n}\n")
    ...
    def write_gravity_live_load(self, building):
        ...
        with open('DefineGravityLiveLoads2DModel.tcl', 'w') as tclfile:
            ...
            tclfile.write("# Define point loads on leaning column\n")
            for i in range(2, building.geometry['number of story']+2):
                tclfile.write("load\t%i%i%i\t0\t[expr -1*$LeaningColumnLiveLoadFloor%i]\t0; \n"
                              % (9, building.geometry['number of X bay']+2, i, i))
            tclfile.write("\n}\n")

            tclfile.write("# puts \"Live load defined\"")

    def write_earthquake_load(self, building, design_method, analysis_load_type, lateral_forces):
        ...
        with open('DefineEarthquakeLaterLoads2DModel.tcl', 'w') as tclfile:
            ...
            tclfile.write("set\tLateralLoad\t[list")
            for i in range(building.geometry['number of story']):
                if design_method == 'RSA' and analysis_load_type == 'EarthquakeLoad':
                    tclfile.write("\t%f"
                                  % (lateral_forces[i] / building.geometry['number of X LFRS'] * ACCIDENTAL_TORSION))
                else:
                    tclfile.write("\t%f"
                                  % (building.seismic_force_for_strength['lateral story force'][i]
                                     / building.geometry['number of X LFRS'] * ACCIDENTAL_TORSION))
    ...
    def write_gravity_earthquake_load(self, building):
        ...
        with open('DefineGravityEarthquakeLoads2DModel.tcl', 'w') as tclfile:
            ...
            tclfile.write("# Define point loads on leaning column\n")
            for i in range(2, building.geometry['number of story']+2):
                tclfile.write("load\t%i%i%i\t0\t[expr -(1.2+0.2*%.2f)*$LeaningColumnDeadLoadFloor%i -"
                              "0.5*$LeaningColumnLiveLoadFloor%i]\t0;\n"
                              % (9, building.geometry['number of X bay']+2, i,
                                 building.elf_parameters['SDS'], i, i))
            tclfile.write("\n")
    ...
    def copy_baseline_files(self, building, design_method, analysis_load_type, which_mode):
        """
        ...
        os.chdir(building.directory['building elastic model'])
        # Revise "EigenValueAnalysis.tcl" file to account for the actual number of modal periods and shapes.
        old_mode = 'set nEigenL 4'
        new_mode = 'set nEigenL 1'
        # If it is RSA-based design, the number of required analysis should be the number of stories.
        if design_method == 'RSA':
            new_mode = 'set nEigenL ' + str(building.geometry['number of story'])
        old_string = '**EIGENVECTOR_NODE**'
        new_string = '111'
        for floor in range(1, building.geometry['number of story'] + 1):
            new_string += (' %i%i%i' % (1, floor + 1, 1))
        ...
        # Revise "Model.tcl" file based on the analysis type that we want to perform.
        ...
        if design_method == 'ELF':
            with open('Model.tcl', 'r') as file:
                content = file.read()
            if analysis_load_type in ['EigenValue', 'DeadLoad', 'LiveLoad', 'EarthquakeLoad', 'GravityEarthquake']:
                new_string = '[list ' + analysis_load_type + ']'
            ...
            with open('Model.tcl', 'r') as file:
                content = file.read()
            if analysis_load_type == 'EarthquakeLoad':
                # Denote which mode the lateral force is
                new_string = ('[list EarthquakeLoad%i]' % which_mode)
                new_content = content.replace(old_string, new_string)
                # Revise the load analysis type in Model.tcl file
                old_load_type = '$LoadType == "EarthquakeLoad"'
                new_load_type = '$LoadType == "EarthquakeLoad' + str(which_mode) + '"'
                new_content = new_content.replace(old_load_type, new_load_type)
            elif analysis_load_type in ['EigenValue', 'DeadLoad', 'LiveLoad']:
                new_string = '[list ' + analysis_load_type + ']'
                new_content = content.replace(old_string, new_string)
            else:
                new_string = '[list DeadLoad LiveLoad]'
                new_content = content.replace(old_string, new_string)
            with open('Model.tcl', 'w') as file:
                file.write(new_content)
