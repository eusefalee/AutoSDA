...
def seismic_design(building_id,base_directory):
    ...
    model_1 = ElasticAnalysis(building_1, DESIGN_METHOD, analysis_load_type='EigenValue')
    ...
    _ = ElasticAnalysis(building_1, DESIGN_METHOD, analysis_load_type='All')

    # Read elastic analysis drift
    building_1.read_story_drift()
    building_1.read_floor_displacement()
    ...
    try:
        ...
        temp = pd.read_csv('DriftLimit.csv', header=0)
        drift_limit = temp.loc[0, 'drift limit']
    ...
    while np.max(building_1.elastic_response['story drift']) * building_1.elf_parameters['Cd'] * RBS_STIFFNESS_FACTOR \
        ...
        _ = ElasticAnalysis(building_1, DESIGN_METHOD, analysis_load_type='EigenValue')
        building_1.read_modal_period()
        building_1.compute_seismic_force()
        # Update the design story drifts
        _ = ElasticAnalysis(building_1, DESIGN_METHOD, analysis_load_type='GravityEarthquake')
        building_1.read_story_drift()
        building_1.read_floor_displacement()
    ...
    for story in range(building_1.geometry['number of story']):
        for connection_no in range(building_1.geometry['number of X bay']+1):
            ...
            while not connection_set[story][connection_no].is_feasible['SCWB']:  # Not feasible connection -> go into loop
    ...
    # Update modal period and seismic forces
    model_2 = ElasticAnalysis(building_2, DESIGN_METHOD, analysis_load_type='EigenValue')
    ...
    _ = ElasticAnalysis(building_2, DESIGN_METHOD, analysis_load_type='All')
    building_2.read_story_drift()
    building_2.read_floor_displacement()
    ...
    # ********************************************************************
    # //////////// Impose Doubler Plate Requirement     //////////////////
    # ********************************************************************
    # By default, the previous design uses doubler plate, which is sometimes costly.
    # If the doubler plate is not allowed, the following codes will be run.
    if not PERMIT_DOUBLER_PLATE:
        for story in range(0, building_2.geometry['number of story']):
            for conn_no in range(0, building_2.geometry['number of X bay']+1):
                # Not permit to use the doubler plate
                while construction_connection_set[story][conn_no].doubler_plate_thickness > 0:
                    print("------------------------- Doubler Plate Opt Out-----------------------------------------")
                    print("Exterior column:", building_2.member_size['exterior column'])
                    print("Interior column:", building_2.member_size['interior column'])
                    print("Beam:", building_2.member_size['beam'])
                    print(construction_connection_set[story][conn_no].shear_force)
                    print("---------------------------------------------------------------------------------------")
                    # Upscale the column that install the doubler plate
                    if conn_no == 0 or conn_no == building_2.geometry['number of X bay']:
                        type_column = 'exterior column'
                    else:
                        type_column = 'interior column'
                    building_2.upscale_column(story, type_column)
                    model_2 = ElasticAnalysis(building_2, DESIGN_METHOD, analysis_load_type='EigenValue')
                    building_2.read_modal_period()
                    building_2.compute_seismic_force()
                    # Re-perform the elastic analysis to obtain the updated demand
    ...
    # Update modal period and seismic forces
    model_3 = ElasticAnalysis(building_3, DESIGN_METHOD, analysis_load_type='EigenValue')
    building_3.read_modal_period()
    building_3.compute_seismic_force()

    # Perform elastic analysis for construction sizes
    _ = ElasticAnalysis(building_3, DESIGN_METHOD, analysis_load_type='All')
    building_3.read_story_drift()
    building_3.read_floor_displacement()
    ...
    save_all_design_results(building_3, construction_column_set, construction_beam_set, construction_connection_set, True)
